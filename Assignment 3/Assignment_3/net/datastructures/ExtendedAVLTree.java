...
import java.awt.*;	//library for plotting
import javax.swing.*;	//library for plotting
...
public class ExtendedAVLTree<K, V> extends AVLTree<K, V>{
	 ...
	 * Time Complexity Analysis
	 * Initialization and return are primitive operations: O(1)
	 * Sorting of the list is done by in-ordering retrieving of the lists:
	 * - for list 1 is O(n)
	 * - for list 2 is O(m)
	 * Therefore, the time complexity for sorting is O(n+m)
	 * Similarly for merging of two lists, the method retrieve all the elements n+m, therefore O(n+m)
	 * Again, to convert list to tree, the method recursively visit all the elements in the Arraylist, hence O(n+m)
	 * In all, the overall complexity is O(n+m)
	 * */
	...
	**/
	private static <K, V> void printTree(BTPosition<Entry<K,V>> node, int x, int y, int space, int height, JFrame frame) {
		int circleHeight = 60 - height*3;	//height of circle
        ...
        if (node.getLeft() != null && node.getLeft().element() != null) { //if there is left child and their elements exist
        	printTree(node.getLeft(), x - space/2, y + MARGIN, space/2, height + 1,frame); // set left child with new axis and height
        } else { 	//if external draw rectangle
            frame.getContentPane().add(new Rectangle(x - space/2, y + MARGIN, rectWidth, rectHeight)); //draw rectangle for external nodes
            frame.setVisible(true);
        }
        
        // Plot right tree; draw rectangle for external nodes
        if (node.getRight() != null && node.getRight().element() != null) { //if there is right child and their elements exist
        	printTree(node.getRight(), x + space/2, y + MARGIN, space/2, height + 1, frame); // set right child with new axis and height
        } else {	//if external draw rectangle
            frame.getContentPane().add(new Rectangle(x + space/2, y + MARGIN, rectWidth, rectHeight));//draw rectangle for external nodes
            frame.setVisible(true);
        }
        ...
        frame.setVisible(true);
        // Print key inside circles
        frame.getContentPane().add(new Text(node.element().getKey().toString(), x - height, y + circleHeight/2));
        frame.setVisible(true);
        // Draw left leg
        frame.getContentPane().add(new Line(x, y + circleHeight, x - space/2, y + MARGIN));
        frame.setVisible(true);
        // Draw right leg
        frame.getContentPane().add(new Line(x, y + circleHeight, x + space/2, y + MARGIN));
        frame.setVisible(true);
	...
	// JComponent drawing classes and functions
